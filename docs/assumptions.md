...
`literature` or `assumption` in its metadata JSON.
...
6. No comparator, FPGA part, or PSU is selected.
...
| A4 | Error-budget term values in three named scenarios | error-budget | Replace every `assumption` term with allocation + measurement |
...
| A7 | Channel offset 8 ps, common 4 ps, independent 6 ps RMS; crosstalk 0.4 ps per extra active channel | channel-scaling | Measure pairwise covariance with 1 vs 16 active |
| A8 | 0.8 V illustrative swing; 0.5 mV threshold uncertainty; 2 ps comparator; 3 ps walk | frontend-jitter | SPICE + datasheet once a part is chosen |
