...
| Current simulation evidence | Coarse+fine arithmetic; parallel-chain SSP model; synthetic calibration; error-budget scenarios; PVT sweeps including a 0.525 ps/°C literature offset scenario; front-end slew contours; `mswu-literature` calculator. Kintex-7 structural P&R: Round 6/7 multichain, Round 8 MSWU structural (historical), Round 9 MSWU validated — **not** S14. |
| Current gap | No physical TDC bins, no closed error budget with measured terms. Vivado WNS on the 4 ns benchmark is capture/control timing, not 1 ps resolution. Round 7 multichain and Round 9 MSWU validated 16ch both met 4 ns — still not metrology. Round 8 MSWU preencoder LUT=3 invalid. |
