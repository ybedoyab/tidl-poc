...
| Current simulation evidence | Coarse+fine arithmetic; parallel-chain SSP model; synthetic calibration; error-budget scenarios; PVT sweeps including a 0.525 ps/°C literature offset scenario; front-end slew contours; `mswu-literature` calculator. Kintex-7 CARRY4 structural P&R: Round 6 (`docs/evidence/vivado_kintex7/`) and timing-clean Round 7 (`docs/evidence/vivado_kintex7_timing_clean/`) are **not** S14. |
| Current gap | No physical TDC bins, no closed error budget with measured terms. Vivado WNS on the 4 ns benchmark is capture/control timing, not 1 ps resolution. Round 6 negative WNS was likely benchmark-only parity logic; Round 7 met 4 ns after removing it — still not metrology. |
